...
describe('Test Suite for Observer Pattern', () => {
	...
	test('#Payment should nofity subject after a credit card transaction', () => {
		const subject = new PaymentSubject();
		const payment = new Payment(subject);

		const paymentSubjectNofifierSpy = jest.spyOn(
			payment.paymentSubject,
			payment.paymentSubject.notify.name
		);
		const data = { userName: 'katrielkoga', id: Date.now() };
		payment.creditCard(data);

		expect(paymentSubjectNofifierSpy).toBeCalledWith(data);
	});
	test('#All should netify subscribers after a credit card transaction', () => {
		const subject = new PaymentSubject();
