...
function productValidator(product) {
	...
	if (!(product.id.length >= 2 && product.id.length <= 20)) {
		errors.push(
			`id: invalid length, current [${product.id}] expect ot be between 2 and 20`
		);
	}
	...
	if (!(product.price >= 1 && product.price <= 1000)) {
		errors.push(
			`price: invalid value, current [${product.price}] expect ot be between 1 and 1000`
		);
	}

	if (!['electronic', 'organic'].includes(product.category)) {
		errors.push(
			`category: invalid value, current [${product.category}] expect ot be either electronic or organic`
		);
	}
	return {
